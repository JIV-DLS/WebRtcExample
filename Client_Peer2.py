...
webrtcbin name=sendrecv bundle-policy=max-bundle stun-server=stun://stun.l.google.com:19302
 uridecodebin uri=rtsp://admin:Greoux04@192.168.110.220 ! videoconvert ! queue ! vp8enc deadline=1 ! rtpvp8pay !
    ...
    async def loop(self):
        ...
        async for message in self.conn:
            if message == 'HELLO':
                ...
                self.start_pipeline()
            elif "sdpMLineIndex" in message:
                ice = json.loads(message)["ice"]
                self.manage_ice_candidate_message(self, ice["sdpMLineIndex"], ice["candidate"])
            elif message.startswith('ERROR'):
                print(message)
                self.close_pipeline()
                return 1
            else:
                print("___ HANDLING SDP ____")
